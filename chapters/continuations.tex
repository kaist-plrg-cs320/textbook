...
\textit{control flow} of programs.\index{control diverter}\index{control flow}
For example, programmers can use \code{return} in Scala
...
prefer \code{return} (\reffig{return}) to calling \code{numOfWords} multiple
...
\textsf{BFAE} interprter in \refch{boxes}.
\begin{verbatim}
...
\code{l} to the evaluation of \code{r}. Store passing is unnecessary since there
...
and the resulting store of \code{l} is used for the evaluation \code{r}.
...
does exist and be executed during the evaluation of \code{factorial(5) + 1},
...
the continuation, which is \code{x => 120 == x}, results in a boolean. The whole
...
The current version is just specification, not implementation, of \code{factorialCps}.
...
the result of the redex. On the other hand, in the small-step semantics,
