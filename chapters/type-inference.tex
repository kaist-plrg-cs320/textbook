...
The type checker infers the type of \code{x} to \code{Int}. However, parameter
...
This section formuates the type inference problem as a decision problem in two
...
For example, \code{Fun("x", VarT("X"), Add(Id("x"), Id("x")))} corresponds to
...
\begin{enumerate}
  \item[a)]
    Fill \C{???} of \C{case (PairT(t3, t4), PairT(t5, t6)) => ???} in \C{unify}
  \item[b)]
    Fill \C{???} of \C{case PairT(l, r) => ???} in \C{occurs}
  \item[c)]
    Fill \C{???} of \C{case Pair(l, r) => ???} in \C{typeCheck}
  \item[d)]
    Fill \C{???} of \C{case Fst(e) => ???} in \C{typeCheck}
  \item[e)]
    Write the result of \C{typeCheck($e$, Map())}, where $e$ denotes
...
\item
\C{TIBFAE} is \C{TIFAE} with boxes. The following Scala
code implements a type checker of \C{TIBFAE}.
\begin{verbatim}
sealed trait Expr
case class Num(n: Int) extends Expr
case class Add(l: Expr, r: Expr) extends Expr
...
\begin{itemize}
\item[a)]
  Define type erasure of the form \framebox{$\emph{erase}(e)=E$} for \texttt{TpolyFAE}.
\end{itemize}
