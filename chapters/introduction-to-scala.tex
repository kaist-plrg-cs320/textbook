...
What is functional programming?\index{functional programming}
...
Firstly,
consider the phrase ``the evaluation of mathematical functions.''
In the perspective of functional programming, a
...
program, a state, which can be interpreted as a memory of a computer,
...
\begin{enumerate}
\item Given a fact that \code{x} equals \code{1}, evaluate
\code{let y = 2 in if y < 3 then x + 4 else x - 5}.
\item Given a fact that \code{x} equals \code{1} and \code{y} equals \code{2},
evaluate \code{if y < 3 then x + 4 else x - 5}.
\item Given a fact that \code{x} equals \code{1} and \code{y} equals \code{2},
evaluating \code{y < 3} yields \code{true} and thus evaluate \code{x + 4}.
\item Given a fact that \code{x} equals \code{1} and \code{y} equals \code{2},
evaluate \code{x + 4}.
...
only pure functions.'' Functional programming avoids mutable variables.
Functional programming does not modify data including variables and objects.
States, which change throughout the execution of programs, do not exist.
...
provide mutable variables or structures like \code{var} of Scala, \code{ref} of
OCaml, and \code{set!} and \code{box} of Racket. However,
functional programming uses immutable things in most cases but can
still express a lot of programs without difficulties.

As we have seen so far,
immutability is the most important concept of functional programming.
Immutability allows modular programming and ease reasoning of programs.
Due to immutability, programs that have to be trustworthy or require parallel computing
...
how to write interesting programs without mutating things.
...
to abstact computation, and pattern matching allows programmers to abstract
...
input and process the input according to mathematical semantics of programming
...
this book use functional programming and Scala, which is a functional language.
...
As Scala programs are compiled to Java bytecode, which runs on Java Virtual
Machines (JVMs), you must
install Java before installing Scala. Java has various versions, and Scala 2.13,
...
provies an installation file for JDK 8.

You can donwload an installation file for Scala 2.13 on the Scala
...
file for SBT can be found on the SBT
website.\sidenote{\url{https://www.scala-sbt.org/download.html}}
Section~\refsec{sbt} will discuss the usage of SBT.
...
The term ``REPL'' stands for ``\textbf{r}ead, \textbf{e}val, \textbf{p}rint, and
\textbf{l}oop''.
It is a program that iterativley reads a line from a user, evaluates the code,
...
Booleans are \code{true} and \code{false} in Scala.
...
REPL omits the result when it is unit.
...
The result of \code{[expression]} becomes the value denoted by the variable and
must be a value of the \code{[type]}.
...
You can omit the \code{: [type]} part and simply write \code{val [name] =
[expression]} instead. In this case, a type mimatch never happens, and the
...
variables. Most variables are immutable in functional programming.
...
Sometimes, mutable variables, i.e. variables whose values can change ease programming.
...
To assign a new value to a mutable variable, the value must follow the
...
Many programming languages require \code{return} to sepcify the return value of a function.
...
Like variables, the return types of functions can be omitted.
...
inside the body of a function, we put expressions separated by line breaks
inside curly braces.
...
not part of code. They has been automatically inserted by REPL.}
...
operator \code{? :} in C than a \code{if} statement.
...
means that the \code{Nil} may come first}
...
The \code{[expression]} in front of \code{match} is the target of pattern matching.
If it is an empty list, it matches \code{case Nil}. The \code{[expression]}
of the \code{Nil} case will be evalauted.
Otherwise, it is a nonempty list and matches \code{case [name] :: [name]}. The first
\code{[name]} denotes the head (the first element) of the list, and the second
\code{[name]} denotes the tail (a list consisting of all the elements except the
head) of the list. The \code{[expression]} of the case will be evalauted.
...
\refch{immutability} will show various use of pattern matching for lists, and
\refch{pattern-matching} will discuss pattern matching in detail.
...
number of elements can vary and an arbitrary index should be able to accessed.
...
It allows us to fetch the height of the \code{i}th
student. On the other hand, tuples are appropriate when the number of elements
are fixed and each index has a specific meaning and own type. For instance,
...
An object is a value with fields and methods. Fields stores values, and methods
...
The first \code{[name]} is the name of a new class. The \code{[name]}s inside
the parentheses are the names of the fields of the class.
...
Creating new objects is similar to function calls.
...
defining a class, that field becomes mutable.
...
An interpreter is a program that takes source code as input and runs the code. A
Scala interpreter takes Scala source code as input. To use the interpreter,
...
your command line. Here, we need \code{scala Hello.scala}.
...
A Scala compiler takes Scala source code as input and translates it into Java
bytecode. Once code is compiled, we can run the generated bytecode with a JVM.

For compilation, we need to define the main method of a program. The main method
is the entrypoint of every program running on JVMs.
...
You can make the compiler compile the code by typing \code{scalac} in your
command line.
...
You can run the bytecode with a JVM by the \code{scala} command. In this time,
it is enough to write only the class name.
...
You can change the behavior of the program by modifying the main method.
...
projects with lots of files by tracking dependencies between files and managing
third-party libraries. There are various build tools in the world, and SBT is
...
The \code{build.sbt} file configures the project. It manages the Scala version
of the project, third-party libraries used in the project, and many other
things.
Source files are under the \code{src} directory. Files under \code{main} are
main source files, while files under \code{test} are only for testing.
You can add files into \code{src/main/scala} and edit files in the directory
to write code.
...
\begin{verbatim}
sbt:hello> compile
[info] compiling 1 Scala source to ~/hello/target/scala-2.13/classes ...

| => root / Compile / compileIncremental 0s
[success] Total time: 4 s
sbt:hello> test
[info] compiling 1 Scala source to ~/hello/target/scala-2.13/test-classes ...
[info] HelloSpec:
