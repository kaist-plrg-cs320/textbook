...
computation of
the return value. Note that vertical bars (\code{|}) at the beginning of lines are
...
in front of an existing list. It does not mutate the existing list to
prepend the new element but rather creates a new list with the element and the list.
...
to fetch the element at a specific index; the first index is \code{0}.
...
name denotes the head---the first element---of the list, and the second
name denotes the tail---a list consisting of all the elements except the
head---of the list. The expression of the \code{::} case will be evaluated.
We can change the order between the cases, thereby making
the \code{::} case come first.
...
To fetch the \code{i}-th element of a tuple, we can use \verb+._i+. The
first index is 1.
...
This book uses only \emph{case} classes of Scala. Case classes are similar to
