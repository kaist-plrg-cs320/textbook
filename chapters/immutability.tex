...
The below \code{factorial} functions calculate the factorial of a given integral
...
The implementation of the \code{factorial} function using recursion is identical
...
The \code{inc1} function takes a list as an argument and returns a list whose
...
The \code{odd} function takes a list as an argument and returns a list whose
...
The \code{length} function calculates the length of a given list.
...
that takes an intermediate result. \code{factorial(n, i)} computes \(n!\times
i\).
...
By revising \code{length}, it becomes tail-recursive:
...
The \code{sum} and \code{product} functions can be modified in the same manner.
...
The following is tail-recursive \code{inc1}.
...
It is possible to implement tail-recursive \code{inc1} using \(O(n)\) time.
...
Define function \code{square}, which takes a list as an argument and returns a
...
Define function \code{positive}, which takes a list as an argument and returns a
...
Define functions \code{sum} and \code{product}, which respectively calculate the
...
Define function \code{addBack}, which takes a list and an integer as arguments
