...
In the first line of the code, \code{x} is \code{1}. Since \code{x} is immutable,
there is no doubt that \code{x} is still \code{1} when \code{x} is passed as an
argument for \code{f} in the last line of the code.
...
modification of \code{x} throughout the code, the value of \code{x} in the last
...
therefore, $\code{res}=(\code{i}-1)!=\code{n}!$ in the last line of the
...
We can see that there is no function call instruction, \code{invokevirtual}.
