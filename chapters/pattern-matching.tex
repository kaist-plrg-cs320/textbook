...
Consider the \verb!list_get! function, which takes a list and integer \code{n} as
arguments and returns the \code{n}th element of the list. The case when \code{n}
...
but use only conventional \code{try-catch} statements to make programs pass type
...
The first attempt is using \code{null}. \code{null} is a value that denotes that
it does not refer to any existing object. In Java and therefore Scala, \code{Int}
is a primitive type, and \code{null} is not an element of \code{Int}. The code is
invalid. Even though we assume that \code{null} belongs to \code{Int}, without
checking whether a return value is \code{null}, using the return value may lead
to \code{NullPointerException}. Like exceptions, \code{null} is beyond the scopes
...
from \code{NullPointerException}. The concept of a nullable type is similar to an
...
The second attempt is using a particular error-indicating value, \code{-1} for
example. The strategy has an obvious problem: at call-sites, programs cannot
judge whether lists contain \code{-1} as elements or indices are wrong. It can be
...
without any mutation. Many languages including Scala use \code{Option} as the
name, but some call them \code{Maybe}. As the name implies, an option type
represents an optional existence of a value. The article defines an option type
for \code{Int} and functions treating values of the type.
...
The code is similar to the code defining \code{List}, \code{Nil}, and
\code{Cons}. A value of the \code{Option} type either is \code{None} or belongs
to \code{Some}. \code{None} is a value that does not denote any value and similar
to \code{null}. It indicates a problematic situation. Like \code{Nil}, it is a
singleton object defined by \code{object}. \code{Some} constructs a value that
...
The following defines \verb!list_getOption! using the \code{Option} type:
...
For wrong indices, the return value is \code{None}. Otherwise, the function packs
an element inside \code{Some} to make the return value.

Define the \code{div100} function, which takes an integer as an argument and
...
distinguishing the \code{None} and the \code{Some} cases. Like the functions for
...
Define the \code{getSquare} function, which takes a list and integer \code{n} as
arguments and returns the square of the \code{n}th element of the list. The
return type of the function must be \code{Option}. Use \verb!list_getOption! and
\verb!option_map!.
...
return type is \code{Option} as the second argument. If every computation
succeeds, the result belongs to \code{Some}. Otherwise, it is \code{None}.

Define the \code{getAndDiv100} function, which takes a list and integer \code{n}
as arguments and returns an integer obtained by dividing \code{100} by the
\code{n}th element of the list. The function must return an optional value. Use
\verb!list_getOption!, \code{div100}, and \verb!option_flatMap!.
