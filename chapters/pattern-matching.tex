...
An expression of the lambda calculus, which is the topic of
\refch{lambda-calculus}, is
...
In addition, becuase of the \code{extends} keyword, \code{Num}, \code{Add}, and
...
inform this knowledge to the compiler with type casting. \code{e.isInstanceOf[Num]}
...
compiler considers \code{e.isInstanceOf[Num]} to belong \code{Num} and allows
...
Wihtout \code{sealed}, a trait can be extended outside the file that defines it.
...
\code{Node(`n`, t0, Empty)} can replace
\code{case None(m, t0, Empty) if n == m}. The pattern \code{Node(n, t0, Empty)} defines
...
Use of \code{null} has been criticized a lot because \code{null} is extremely
