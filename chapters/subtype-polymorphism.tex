...
\begin{itemize}
  ...
  \item \code{subtype($\tau_1$, $\tau_2$)} is \code{false} when $\subt{\tau_1}{\tau_2}$
    is false.
  \item \code{subtype($\tau_1$, $\tau_2$)} always terminates and never throws an
    exception.
\end{itemize}

Fill every \code{???} to complete the implementation.  You may use the
\code{get}\sidenote{\url{https://www.scala-lang.org/api/current/scala/collection/Map.html\#get(key:K):Option[V]}}
and
\code{forall}\sidenote{\url{https://www.scala-lang.org/api/current/scala/collection/Map.html\#forall(p:A=\%3EBoolean):Boolean}}
methods of \code{Map}:

\item Many real-world languages, such as Scala and TypeScript, provide
...
\item The following code implements a type checker for a variant of \lang:
