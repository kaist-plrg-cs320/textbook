...
However, records help programmers to express their high-level ideas in the code
more directly than tuples and, thus, prevent mistakes.

For example,
consider a tuple that represents the height, score, and scholarship state of a
...
However, there is no reason to assoicate the first element to the height. If he or
...
be the set of every label and metavariable $l$ range over labels.
...
\begin{itemize}
  \item
    ...
    Since a record can has zero fields, $\{\}$ is an expression.
  \item
...
To make language support records, we should add record values to the
language.
...
$\{\ca:\tnum,b:\tnum\}$. In addition, the type of the expression
$\{\ca=1+2,\cb=3+4\}$, which evaluates to $\{\ca=3,\cb=7\}$, also is
$\{\ca:\tnum,b:\tnum\}$. Similarly, the type of the empty record is $\{\}$.
...
includes any records that have a integer-valued field \code{a}. Records that
...
\typerule{Typ-Record'}{
  If
    ...
    \typeofdn{e_m}{\tau_m}, \\
  then
    \typeofdn{\{l_1=e_1,\cdots,l_n=e_n,\cdots,l_m=e_m\}}{\{l_1:\tau_1,\cdots,l_n:\tau_n\}}.
}
...
\[
  ...
  { \typeofd{e_1}{\tau_1} \\ \cdots \\
    ...
    \typeofd{e_m}{\tau_m} }
  { \typeofd{\{l_1=e_1,\cdots,l_n=e_n,\cdots,l_m=e_m\}}{\{l_1:\tau_1,\cdots,l_n:\tau_n\}} }
  \quad\textsc{[Typ-Record']}
\]
...
becomes well-typed. However, the first function becomes ill-typed. There is no
...
example, any values that belongs to $\{\ca:\tnum,\cb:\tnum\}$ are values of
...
evaluates to a value of \code{Cat}, then it can be considered as an expression
that evaluates to a value of \code{Animal} and safely given to the function that
represents the hospital. On the other hand, the inverse is false. If a hospital
...
function application is ill-typed, while the the second function application is
...
the tope type. The bottom type denotes the empty set. Since the empty set is a subset of
...
expressions do not evaluate to any values. They just change the control flow.
