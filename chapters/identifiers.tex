...
the shadowing. On the other hand, \code{x} at line 4 denotes the value of an
...
related to free identifiers. If a variable named \code{y} is is free in
...
\semanticrule{Id}{
If
  $x$ is an element of the domain of $\sigma$,\\
then
  \evaldn{x}{\sigma(x)}.
}
...
the environment. By adding the fact to $\sigma$, we get $\sigma[x\mapsto v_1]$.
As $e_2$ is the scope of $x$, $e_2$ is evaluated under $\sigma[x\mapsto v_1]$.
The result of $e_2$ is the final result.

This semantics naturally explains shadowing. Let $x$ already be in the domain of
$\sigma$. Suppose that $\sigma(x)=v$. However, $e_2$ is evaluated under
$\sigma[x\mapsto v_1]$, and $\sigma[x\mapsto v_1](x)=v_1$. When $x$ is used in
$e_2$, its value is $v_1$, not $v$. Therefore, we can say that the
...
We can express them as the inference rules:
...
The standard library provides various methods for maps.
We use two methods to implement the interpreter.

\begin{itemize}
  \item \code{+}

    It allows us to add the pair of a key and a value to the map. For example,
    where \code{m} is \code{Map(1 -> "one")}, we \code{m + (2 -> "two")} is the same as
    \code{Map(1 -> "one", 2 -> "two")}

  \item \code{getOrElse}

    It takes two arguments. The first argument is a key, and the second argument
    is a default value. If the map has the key, then the result is the
    corresponding value. Otherwise, the default value is returned.
    For instance, where \code{m} is \code{Map(1 -> "one")}, \code{m.getOrElse(1,
    "?")} is \code{"one"}, and \code{m.getOrElse(2, "?")} is
    \code{"?"}.
\end{itemize}
...
is nothing much to explain. In the \code{Id} case, we use the function
\code{error}, which is defined in \refsec{options}. When \code{x} is a key in
\code{env}, the corresponding value becomes the result of \code{interp}.
Otherwise, \code{error}, which throws an exception, is called, and the execution
...
\begin{enumerate}
  \item $\ebind{\cx}{(\ebind{\cx}{3}{\esub{5}{x}})}{\eadd{1}{\cx}}$
