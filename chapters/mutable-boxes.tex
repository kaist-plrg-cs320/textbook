...
a reference in OCaml (\code{ref}). However, it is good abstraction of more
general mutation mechanism including mutable objects and data
...
\begin{itemize}
  ...
  \item $\ederef{e}$
    ...
    It read the value of a box, cf. \code{b.value} in the example.
...
We call the memory of a program a store. A \textit{store}\index{store} records the values of boxes.
...
A change in an environment is propagated to the subexpressions of the expression
