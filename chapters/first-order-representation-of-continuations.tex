...
as C, lack first-class functions.\sidenote{C provides functions pointers, but not
...
This chapter hows how we can represent continuations without
...
\begin{itemize}
  \item Previous: \code{v1 => interp(e2, env, v2 => continue(k ,numVAdd(v1, v2)))} and
...
\begin{itemize}
  \item Previous: \code{v2 => continue(k, numVAdd(v1, v2))} and \code{v2} are given.
...
function application. It evaluates the argument and applies the function (or
the continuation) to the argument.
...
evaluation of the argument of function application. It applies the function (or
the continuation) to the argument.
...
\code{interp}. It is one that is represented as the identity function and is
passed to \code{interp} in the beginning. The identity function returns a given
argument without any changes. No addition information is necessary to determine the
...
In summary, the \lang interpreter consists of continuations of the following five
sorts:
...
\code{DoAddK(v1, k)} represents \code{v2 => continue(k, numVAdd(v1, v2))}, and
