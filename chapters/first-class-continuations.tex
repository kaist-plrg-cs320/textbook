...
continuation.\sidenote{Like before, the line below $1$ denotes that $1$ is an
...
continuations.\sidenote{We omit the common part to \plang.}
...
The \code{interp} function must handle continuation application in the
\code{App} case.
...
When \code{v1} is a \code{CloV} instance, the interpreter behaves just like before.
If \code{v1} is a \code{ContV} instance, the expression applies the continuation
to the argument. The applied continuation is the field of \code{v1}, and the argument
is \code{v2}. Since the interpreter expresses a continuation with a Scala function,
applying the Scala function, the field of \code{v1}, to \code{v2} is enough.
...
\code{break}, and \code{continue}, to allow programmers to change the control
...
Expressions like \code{return} change the control flow in a fixed way according
...
A \code{return} expression makes the function immediately return.
...
\sidenote{Assume that the rest of the expression does not use $\code{r}$ at all.}
...
use them inside loops to change the control flow. A \code{break} expression
...
\sidenote{Assume that the rest of the expression does not use $\code{b}$ at all.}
...
Then, the \code{b} is the continuation finishing the evaluation.
Inside the loop, \code{b} is applied to $0$, so the program terminates and
produces $0$ as a result. It coincides the expected behavior of \code{break}.
...
\sidenote{Assume that the rest of the expression does not use $\code{c}$ at all.}
