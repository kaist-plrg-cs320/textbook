...
$x_2$, and body is $e$. In many real-world languages, a function have zero or
...
function call expressions. The following is the abstract syntax of \lang:
\sidenote{We omit the common part to \plang.}

\[ e\ ::=\ \cdots\ |\ \eappfo{x}{e} \]

$x(e)$ is a function call expression. It calls a function named $f$. $e$
determines the value of the argument of the call.
...
\semanticrule{Val}{
If
  \evaln{\sigma\tand\Lambda}{e_1}{v_1}, and
  \evaln{\sigma[x\mapsto v_1]\tand\Lambda}{e_2}{v_2},\\
then
  \evaln{\sigma\tand\Lambda}{\ebind{x}{e_1}{e_2}}{v_2}.
}
...
\[
  \inferrule
  {
    \eval{\sigma,\Lambda}{e_1}{v_1} \\
    \eval{\sigma[x\mapsto v_1],\Lambda}{e_2}{v_2}
  }
  { \eval{\sigma,\Lambda}{\ebind{x}{e_1}{e_2}}{v_2} }
  \quad\textsc{[Val]}
\]
...
def interp(e: Expr, env: Env, fEnv: FEnv): Int = e match {
  ...
  case App(f, a) =>
    val (x, e) = fEnv.getOrElse(f, error("free identifier"))
