...
function. This observation let us know that an anonymous function consists of
...
\begin{itemize}
  \item $\efun{x}{e}$
    ...
    A function have zero or more parameters in many real-world languages,
...
function. Thus, we define a new kind of semantic elements, values. The
...
functions can move around the program. However, the environments used for the
...
not an integer, to make the execution succeeds.
...
by the parser of \lang with named local functions imbeded in the semantics.
...
\]
Describe a trace of the evalaution in terms of arguments to an \code{interp}
...
For \code{Num}, \code{Id}, and \code{Fun} expressions, show the result value, which
is immediate. You can use the following abbreviations and possibly more abbreviations:
...
\item Rewrite the following \lang expression to \textsf{FAE} expression.
  You should not ``evaluate'' them. Consider the approach of \refsec{fae}.
...
Consider we extend value of \lang to include a special value $\uparrow$
...
\begin{itemize}
\item If every free identifier of $e$ is in $\sigma$ or is $x$,
...
\item This exercise replaces the environment-based semantics of \lang with subtitution-based semantics.
