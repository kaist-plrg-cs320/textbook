...
\[
\begin{array}{@{}rrl@{\hskip40pt}rrl@{\hskip40pt}rrl@{}}
  e &::=& n & c &::=& \textsf{skip} & b &::=& \textsf{true} \\
  &|& b &&|& \eset{x}{e} &&|& \textsf{false} \\
  &|& x &&|& \eif{e}{c}{c} & v &::=& n\\
  &|& \eadd{e}{e} &&|& \textsf{while}\ e\ c &&|& b\\
  &|& e<e &&|& \eseq{c}{c} & \tau &::=& \tnum \\
  &&&&&&&|& \tbool \\
\end{array}
\]

Metavariable $c$ ranges over commands.  Under a given environment $\sigma$,
evaluation of an expression yields a value and does not change $\sigma$. On the
other hand, evaluation of a command produces a new environment. The operational
...
\begin{array}{@{}c@{}}
  ...
  { \evald{e}{\textsf{true}} \\
    \evald{c}{\sigma_1} \\
    \eval{\sigma_1}{\textsf{while}\ e\ c}{\sigma_2} }
  { \evald{\textsf{while}\ e\ c}{\sigma_2} }
  \\[2em]
  \inferrule
  { \evald{e}{\textsf{false}} }
  { \evald{\textsf{while}\ e\ c}{\sigma} }
  \qquad
  \inferrule
  { \evald{c_1}{\sigma_1} \\
    \eval{\sigma_1}{c_2}{\sigma_2} }
...
\item Write typing rules of the form \fbox{$\typeofd{c}{\Gamma}$}.
  The following command must be well-typed:
    $\eseq{\eset{\cx}{0}}{\eset{\cx}{1}}$.
    However, the following command must be not well-typed:
    $\eseq{\eset{\cx}{0}}{\eset{\cx}{\textsf{true}}}$.
\end{enumerate}
